...
const totalPages = computed(() => {
  return Math.ceil(displayJobs.value.length / itemsPerPage.value)
})
...
const displayJobs = computed(() => {
  ...
  //  ไม่มี category → ใช้ปกติ
  if (!selected.length) return filteredJobs.value

  // มี category → sort ก่อน
  return [...filteredJobs.value].sort((a, b) => {
    const countMatch = (job) =>
      job.roles.filter(r => selected.includes(r.name)).length
  ...
  <div class="px-1 md:px-4 xl:px-6 2xl:px-0 md:max-w-350 md:mx-auto md:w-full">
    ...
    <div class="flex items-center gap-2 sm:gap-3 w-full max-w-2xl mx-auto">
      ...
      <button @click="showCategory = true" 
        class="bg-orange-gradient text-white px-4 sm:px-6 py-2.5 rounded-full hover:shadow-lg transition-shadow cursor-pointer text-[14px] font-bold shrink-0 tracking-wide">
        Suggest
      </button>
        ...
        <div class="flex-1 flex flex-col min-h-[73vh]">
          ...
          <div class="flex items-center justify-between mb-3 px-1">
            ...
            <label class="flex items-center gap-2 cursor-pointer group select-none">
              <span class="text-[13px] font-bold transition-colors"
                :class="showFavoritesOnly ? 'text-[#EF7722]' : 'text-gray-400 group-hover:text-gray-600'">Favorites</span>
              <div class="relative w-10 h-[22px] rounded-full p-0.5 transition-colors duration-300 border"
                :class="showFavoritesOnly ? 'bg-orange-50 border-orange-200' : 'bg-gray-100 border-gray-200'">
                <input type="checkbox" v-model="showFavoritesOnly" class="hidden">
                <div
                  class="w-[16px] h-[16px] rounded-full shadow-sm transition-transform duration-300 flex items-center justify-center transform"
                  :class="showFavoritesOnly ? 'translate-x-[18px] bg-[#EF7722]' : 'translate-x-0 bg-white border border-gray-300'">
                  <svg v-if="showFavoritesOnly" class="w-2.5 h-2.5 text-white" fill="currentColor" viewBox="0 0 24 24">
                    <path
          ...
          <div class="flex flex-col gap-2">
            ...
            <div v-if="paginatedJobs.length === 0"
              class="flex flex-col items-center justify-center py-12 px-4 bg-[#F9FAFB]/80 rounded-2xl border border-gray-100 text-center">
        ...
        <div
          ...
          <div class="flex flex-col gap-3">
            <h3 class="text-[15px] font-bold text-[#1F2937] leading-snug">
              My Upcoming Events <span class="text-[12px] font-medium text-gray-400 whitespace-nowrap ml-0.5">(7
                days)</span>
            </h3>
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showCategory" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <!-- Popup Component -->
        <CategoryPopup class="relative z-10 w-full max-w-[650px] popup-content" @close="showCategory = false" @save="handleCategorySave"/>
      </div>
    </Transition>
  </Teleport>
